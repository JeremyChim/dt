...
from script.ability_script import update_charge_restore, update_value_more, update_value, expand_per, expand_per2
            ...
            try:
                if ch == 1:
                    ...
                    if cd == 1 or 'Cooldown' in x or 'ManaCost' in x or 'CastPoint' in x or 'RestoreTime' in x:
                        x = x.replace(expand_per, '-12.5%').replace(expand_per2, '-25%')
                    elif va == 1:
